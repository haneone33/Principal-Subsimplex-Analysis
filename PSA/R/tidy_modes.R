...
#'
#' @param result a list generated by [`psa()`].
...
tidy_modes <- function(result, ..., dimensions = NULL){
  ...
  lapply(result$modes[paste0("r=", dimensions)], function(m){cbind(as.data.frame(m), ...)}) |>
    ...
    dplyr::mutate(Dimension = factor(dplyr::.data$Dimension, levels = paste0("r=", dimensions), ordered = TRUE))
}
...
ggmodes <- function(result, ..., dimensions = NULL,
  ...
  baseplt <- tidyr::pivot_longer(modes, colnames(result$pts[[1]]), names_to = "Component") |>
    ggplot2::ggplot(ggplot2::aes(x = .data$Component, y = .data$value)) +
    ggplot2::facet_wrap(dplyr::vars(dplyr::.data$Dimension), scales="free")
...
ggscores_pairs <- function(result, ..., dimensions, mapping = NULL){
  ...
  ggplot2::ggplot(scores, mapping = basemapping) +
    geom_point(mapping = mapping) +
    geom_text(aes(x = .data$x, y = .data$y, label = .data$label), data = labels)
}
